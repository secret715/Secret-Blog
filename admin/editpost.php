...
if(isset($_POST['title']) && isset($_POST['content']) && isset($_POST['keyword']['0']) && trim($_POST['keyword']['0'])!='' && trim($_POST['title'])!='' && trim($_POST['content'])!=''){
	$keyword=implode(',',$_POST['keyword']);
	
	if($_SESSION['Blog_UserGroup']<5){
		$SQL->query("UPDATE `post` SET `title`='%s', `content`='%s', `public`='%d', class='%d', `keyword`='%s' WHERE `id`='%d' AND `author`='%d'",array(htmlspecialchars($_POST['title']),$_POST['content'],$_POST['public'],$_POST['class'],htmlspecialchars(trim($keyword,',')),abs($_GET['id']),$_SESSION['Blog_Id']));
	}else{
		$SQL->query("UPDATE `post` SET `title`='%s', `content`='%s', `public`='%d', class='%d', `keyword`='%s' WHERE `id`='%d' AND (`public`!=0 OR `author`='%d')",array(htmlspecialchars($_POST['title']),$_POST['content'],$_POST['public'],$_POST['class'],htmlspecialchars(trim($keyword,',')),abs($_GET['id']),$_SESSION['Blog_Id']));
	}
	$_ok=true;
}
...
<script>
$(function(){
	sb_filemanager(<?php echo $post['row']['id']; ?>);
	sb_uploader(<?php echo $post['row']['id']; ?>,2000*1000);
});
</script>
<?php if(isset($_ok)){ ?>
	...
	<div class="row">
		<div class="col-md-9">
			<fieldset>
				...
				<textarea class="form-control ckeditor" name="content"><?php echo stripslashes($post['row']['content']); ?></textarea>
			</fieldset>
		</div>
		<div class="col-md-3">
			<fieldset>
